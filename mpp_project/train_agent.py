...
PHASE3_MODEL_PATH = os.path.join(MODELS_DIR, "ppo_phase3_all_random.zip")
...
PHASE2_STEPS = 3_500_000
PHASE3_STEPS = 5_000_000
...
def run_training_curriculum():
    ...
    if not os.path.exists(PHASE1_MODEL_PATH):
        ...
        model = PPO("MlpPolicy", env_phase1, verbose=0, **HYPERPARAMS)
    ...
    if not os.path.exists(PHASE3_MODEL_PATH):
        ...
        model = PPO.load(PHASE2_MODEL_PATH, env=env_phase3, **HYPERPARAMS)
        logger_phase3 = create_logger("Phase3_AllRandom")
    ...
    if not os.path.exists(FINAL_MODEL_PATH):
        ...
        model = PPO.load(PHASE3_MODEL_PATH, env=env_phase4, **HYPERPARAMS)
